...
double VelocityProfileGenerator::calc_distance(const double& v_i,
  ...
  if (std::abs(a) < DBL_EPSILON) {
    ...
    // the division by 0 case was already handled for us by the initial code above, in the if situation
...
double VelocityProfileGenerator::calc_final_speed(const double& v_i,
  ...
  // the negative case or the infinity one was already handled for us by the initial code
